...
This routine stores the result, and as the signature indicates, it overwrites both c and d.

*/
void tridiagonalSolver(real1d const &a, real1d const &b, real1d &c, real1d &d, int const n) {
  c(0) /= b(0);
  d(0) /= b(0);
  for (int i = 1; i < n-1; i++) {
    c(i) /= b(i) - a(i)*c(i-1);
...
This routine stores the result, and as the signature indicates, it overwrites b, c, d, and tmp.
...
*/
inline void tridiagonalSolverPeriodic(real1d const &a, real1d &b, real1d &c, real1d &d, real1d &tmp, int const n) {
  ...
  tmp(0) *= div;
  for (int i = 1; i < n-1; i++) {
    div = 1._fp / (b(i) - a(i)*c(i-1));
    c  (i) =  c(i)                  * div;
    d  (i) = (d(i) - a(i)*d  (i-1)) * div;
    tmp(i) = (     - a(i)*tmp(i-1)) * div;
  }
  div = 1._fp / (b(n-1) - a(n-1)*c(n-2));
  ...
  real factor = (d(0) - a(0)*d(n-1)/b0)/(tmp(0) - a(0)*tmp(n-1)/b0 + 1._fp);
