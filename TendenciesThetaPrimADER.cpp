...
void TendenciesThetaPrimADER::compEulerTend_X(realArr &state, Domain const &dom, Exchange &exch, Parallel const &par, realArr &tend) {
...
void TendenciesThetaPrimADER::compEulerTend_Y(realArr &state, Domain const &dom, Exchange &exch, Parallel const &par, realArr &tend) {
  auto &stateLimits = this->stateLimits;
  auto &fwaves      = this->fwaves     ;
  auto &src         = this->src        ;
  auto &stateGLL    = this->stateGLL   ;
...
void TendenciesThetaPrimADER::compEulerTend_Z(realArr &state, Domain const &dom, Exchange &exch, Parallel const &par, realArr &tend) {
  auto &stateLimits = this->stateLimits;
  auto &fwaves      = this->fwaves     ;
  auto &src         = this->src        ;
  auto &stateGLL    = this->stateGLL   ;
...
void TendenciesThetaPrimADER::compEulerTend_S(realArr &state, Domain const &dom, Exchange &exch, Parallel const &par, realArr &tend) {
  // Form the tendencies
  // for (int k=0; k<dom.nz; k++) {
  //   for (int j=0; j<dom.ny; j++) {
  //     for (int i=0; i<dom.nx; i++) {
...
void TendenciesThetaPrimADER::edgeBoundariesZ(realArr &stateLimits, realArr &fluxLimits, Domain const &dom) {
  // for (int j=0; j<dom.ny; j++) {
  //   for (int i=0; i<dom.nx; i++) {
  yakl::parallel_for( dom.ny*dom.nx , YAKL_LAMBDA (int const iGlob) {
    int j, i;
...
void TendenciesThetaPrimADER::compStrakaTend(realArr &state, Domain const &dom, Exchange &exch, Parallel const &par, realArr &tend) {
  //Exchange halos in the x-direction
  exch.haloInit      ();
  exch.haloPackN_x   (dom, state, numState);
  exch.haloExchange_x(dom, par);
