...
void TendenciesThetaPrimADER::initialize(Domain const &dom) {
  ...
  SArray<real,2,tord,tord> g2c, c2d, c2g;
...
void TendenciesThetaPrimADER::compEulerTend_X(real4d &state, Domain const &dom, Exchange &exch, Parallel const &par, real4d &tend) {
...
void TendenciesThetaPrimADER::compEulerTend_Y(real4d &state, Domain const &dom, Exchange &exch, Parallel const &par, real4d &tend) {
  auto &stateLimits = this->stateLimits;
  auto &fwaves      = this->fwaves     ;
  auto &src         = this->src        ;
  auto &stateGLL    = this->stateGLL   ;
...
void TendenciesThetaPrimADER::compEulerTend_Z(real4d &state, Domain const &dom, Exchange &exch, Parallel const &par, real4d &tend) {
  auto &stateLimits = this->stateLimits;
  auto &fwaves      = this->fwaves     ;
  auto &src         = this->src        ;
  auto &stateGLL    = this->stateGLL   ;
...
void TendenciesThetaPrimADER::compEulerTend_S(real4d &state, Domain const &dom, Exchange &exch, Parallel const &par, real4d &tend) {
  // Form the tendencies
  // for (int k=0; k<dom.nz; k++) {
  //   for (int j=0; j<dom.ny; j++) {
  //     for (int i=0; i<dom.nx; i++) {
...
void TendenciesThetaPrimADER::stateBoundariesZ(real4d &state, Domain const &dom) {
  // for (int j=0; j<dom.ny; j++) {
  //   for (int i=0; i<dom.nx; i++) {
...
void TendenciesThetaPrimADER::edgeBoundariesZ(real5d &stateLimits, real5d &fluxLimits, Domain const &dom) {
  // for (int j=0; j<dom.ny; j++) {
  //   for (int i=0; i<dom.nx; i++) {
  parallel_for( Bounds<2>(dom.ny,dom.nx) , YAKL_LAMBDA (int j, int i) {
    stateLimits(idR ,0,0     ,j,i) = stateLimits(idR ,1,0     ,j,i);
...
void TendenciesThetaPrimADER::compStrakaTend(real4d &state, Domain const &dom, Exchange &exch, Parallel const &par, real4d &tend) {
  //Exchange halos in the x-direction
  exch.haloInit      ();
  exch.haloPackN_x   (dom, state, numState);
  exch.haloExchange_x(dom, par);
